...
You'll end up able to run `python src/agent.py` and ask *"what's that noise?"* — a local LLM that
...
./.venv/bin/python src/features.py --seconds 2
...
```bash
./.venv/bin/python src/agent.py --model qwen2.5:1.5b
./.venv/bin/python src/capture_test.py --device 2      # use input device index 2
```
...
```bash
export OLLAMA_HOST=http://ollama:11434     # then run src/agent.py
```
> Note: `src/agent.py` currently uses the default `ollama.Client()` (localhost). If you deploy with
...
PYTHONPATH=src ./.venv/bin/python -c "import numpy as np, json; from scipy.io import wavfile; \
